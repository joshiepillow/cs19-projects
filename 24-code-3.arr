...
fun how-many-24-10-4() -> Number:
  doc: ```calculate the number of 4 card 3 operation configurations that result in a value of 24, 
       where the cards are between 1 and 10, and the operations come from {add, sub, mul, div}```
  number-alphabet = generate-pool(range(1, 11), 4)
  operation-alphabet = generate-pool([list: add, sub, mul, div], 3)
